...
const Array<int>& SpatialHashGrid::getStartIndices() {
    return startIndices;
}

const Array<int>& SpatialHashGrid::getEndIndices() {
    return endIndices;
}

void SpatialHashGrid::setCellDirty(int cellHash) {
    dirty[cellHash] = true;
}

bool SpatialHashGrid::isCellDirty(int cellHash) {
    return dirty[cellHash];
}

bool SpatialHashGrid::isValidCellPos(int2 cellPos) {
    return (cellPos.x >= 0 && cellPos.x < gridWidth && cellPos.y >= 0 && cellPos.y < gridHeight);
}

bool SpatialHashGrid::isValidPos(Vector2 pos) {
    return (pos.x >= m_pos.x && pos.x < m_pos.x + m_size.x && pos.y >= m_pos.y && pos.y < m_pos.y + m_size.y);
}

int2 SpatialHashGrid::getCellPos(int cellHash) {
    return { cellHash % gridWidth, cellHash / gridWidth };
}
...
int SpatialHashGrid::getCellHash(int2 cellPos) {
    return (cellPos.x + cellPos.y * gridWidth);
}

void SpatialHashGrid::generateHashList(const Array<Critter>& critters, Array<int> IDs) {
...
void SpatialHashGrid::draw(const Array<Critter>& critters) {
